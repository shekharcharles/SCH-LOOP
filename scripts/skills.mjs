...
export const SCH_COMMANDS = [
  ...
  { name: "run", status: "implemented", routes_to: "skill:sch-run", summary: "run one pass of the in-session loop", note: "for ONE task in a fresh EXTERNAL claude process use sch-run-task.mjs, then sch-deliver-run.mjs to commit and push it; sequential queue continuation and automatic retry are not implemented", milestone: "sequential queue-driven task graph" },
