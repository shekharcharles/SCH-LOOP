...
export const SCH_COMMANDS = [
  ...
  { name: "workflows", status: "implemented", routes_to: "state.mjs workflow-template-list / workflow-template-show / workflow-template-validate", summary: "the versioned workflow templates and the closed handler registry they are validated against", note: "a template is DATA — it can never grant a tool, widen a write scope or name a module" },
  { name: "roles", status: "implemented", routes_to: "state.mjs role-list / role-show / role-validate / role-resolve", summary: "the agent-role roster and logical model profiles, and what a role would actually resolve to for a task", note: "role, executor, provider, model, tools and write scope are six separate things; a skill widens none of them" },
  { name: "trace", status: "implemented", routes_to: "state.mjs workflow-status / workflow-trace --task <n>", summary: "which workflow a task ran and every phase, actor lane and gate inside it" },
  { name: "usage", status: "implemented", routes_to: "state.mjs usage-show", summary: "usage, cost and latency per task, role and workflow", note: "UNKNOWN is reported as UNKNOWN and counted separately — it is never summed as zero" },
  { name: "skill-sources", status: "implemented", routes_to: "state.mjs skill-source-add / skill-source-list / skill-source-sync / skill-source-disable", summary: "governed EXTERNAL skill repositories, pinned to a full commit and synced only by an operator", note: "no auto-update, no auto-trust, no global install; a worker can never fetch a source" },
  { name: "external-skills", status: "implemented", routes_to: "state.mjs external-skill-list / -show / -review / -approve / -disable / -update-diff / -conflicts", summary: "inventory, risk, quality, conflicts and hash-bound ROLE-SCOPED approval for external skills", note: "default eligibility is nothing; a quality pass is not an approval; push and scheduling skills are never eligible for a worker" },
  { name: "test-suite", status: "implemented", routes_to: "sch-test.mjs [--focused <files>] [--status] [--release]", summary: "run the test suite under a lease so two complete suites can never overlap", note: "a full suite has a heartbeat and an explicit outer timeout — a quiet run is no longer mistaken for a hung one" },
